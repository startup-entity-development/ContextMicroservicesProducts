...
class ShopperDataBase(ObjectDataBase):
    ...
    def create(self, shopper:ShopperAttributesGateway, auto_commit:bool= True, raise_integrety_except=False) -> Optional[Shopper]:
        """Create shopper."""
        try:
    ...
    def delete_all(self, auto_commit:bool= True) -> None:
        """Delete all shoppers."""
        try:
            self.db_session.query(Shopper).delete()
            if auto_commit:
                self.db_session.commit()
    ...
    def get(self, id:int) -> Shopper:
        """Get shopper model."""
        try:
            shopper = self.db_session.query(Shopper).filter_by(id=id).first()
            self.log.info(f"get_shopper done")
            return shopper
        except Exception as e:
            self.log.error(f"file: setup_db.py method:, get_shopper fail {e}")     
    
    def get_all(self) -> List[Shopper]:
        """Get all shoppers model."""
        try:
            shoppers = self.db_session.query(Shopper).all()
    ...
    def delete(self, id_shopper:int, auto_commit:bool= True):
        # delete shopper
        try:
            shopper = self.db_session.query(Shopper).get(id_shopper)
