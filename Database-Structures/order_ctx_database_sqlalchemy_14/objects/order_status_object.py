...
class OrderStatusDataBase(ObjectDataBase):
    ...
    def get(self, id:int) -> OrderStatus:
        """Get order_status model."""
        try:
            order_status = self.db_session.query(OrderStatus).filter_by(id=id).first()
    ...
    def get_all(self) -> List[OrderStatus]:
        ...
        try:
            order_statuss = self.db_session.query(OrderStatus).all()
    ...
    def delete(self, id_order_status:int, auto_commit:bool= True):
        # delete order_status
        try:
            order_status = self.db_session.query(OrderStatus).get(id_order_status)
