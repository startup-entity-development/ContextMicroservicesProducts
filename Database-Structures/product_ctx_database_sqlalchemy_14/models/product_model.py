...
from sqlalchemy import Column, String, ForeignKey, Integer, Float, Index,Computed,Enum
...
from product_ctx_database_sqlalchemy_14.models.types.ts_vector_type import TSVector 
...
class ModelProduct(Base):
    ...
    def subcategory_name(context):
        """Return the name of the subcategory."""
        try:
            id_subcategory = context.current_parameters["sub_category_id"]
        except KeyError:
            id_subcategory = context.current_parameters["Product_sub_category_id"]
        except Exception as e:
            return ""
        sub_category_db:ModelSubCategory = db_session.query(ModelSubCategory).filter(ModelSubCategory.id == id_subcategory).first()
        return sub_category_db.name
    ...
    title = Column(String(250), unique=True, doc="Title of the Product")
    words_tags = Column(String(250), nullable=False,
                         default="",
                         onupdate="" ,
                        doc="TagsWords of the Product")
    brand = Column(String(100), nullable=True, doc="Brand of the Product")
    base_increment = Column(Float, doc="Increment of the Product", default=1)
