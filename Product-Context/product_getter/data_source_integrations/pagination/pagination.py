...
class PaginationFunctionClass(ABC):
    ...
    def __init__(self) -> None:
        self.list_of_paginations:List[Pagination] = []
    ...
    def initital_pagination(self,initial_page:Pagination) -> None:
        """Set the initial pagination"""
        self.list_of_paginations.append(initial_page)
    ...
    def save_list_of_initial_paginations(self, file_name:str="initial_paginations.json"):
        """Save a list of links to be used for pagination"""
        with open(file_name, "w") as file:
                file.truncate(0)
                file.write("[")
                for page in self.list_of_paginations:
                    file.write(page.json_string)
                    if page != self.list_of_paginations[-1]:
                        file.write(",")
                file.write("]")
