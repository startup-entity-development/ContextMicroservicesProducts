...
class RetailerDatabase(ObjectDataBase):
    ...
    def get_by_api_name(self, api_name: str, only_active:False) -> List[ModelRetailer]:
        self.log.info("Getting retailer by api name")
        try:
            if only_active:
                retailers = self.db_session.query(ModelRetailer).filter(ModelRetailer.api_name == api_name, ModelRetailer.is_active == True).all()
            else:
