...
from product_ctx_database_sqlalchemy_14.objects.utils import UnitMeasure, normalize_string
...
class ProductAttributes(): 
    ...
    def __cut_string(self, string:str, length:int)->str:
    ...
    def model_dictionary(self) ->dict:
        ...
        obj_dict["api_id"] = self.__cut_string(obj_dict["api_id"], 300)
        obj_dict["name"] = self.__cut_string(obj_dict["name"], 200)
        obj_dict["title"] = self.__cut_string(obj_dict["title"], 250)
        obj_dict["brand"] = self.__cut_string(obj_dict["brand"], 100)
        obj_dict["description"] = self.__cut_string(obj_dict["description"], 2500)
...
class ProductDatabase(ObjectDataBase):
    ...
    def delete(self, id:int, auto_commit:bool= True) -> None:
        ...
        try:
            ...
            if auto_commit:
                self.db_session.commit()
            self.log.info("delete_product done")
        except Exception as e:
    ...
    def get(self, id:int) -> ModelProduct:
        ...
        try:
            product = self.db_session.query(ModelProduct).filter_by(id=id).first()
            self.log.info("get_product_by_id done")
            return product
        except Exception as e:
    ...
    def get_by_tcin(self, tcin:int) -> ModelProduct:
        ...
        try:
            ...
            self.log.info("get_product_by_tcin done")
            return product
        except Exception as e:
    ...
    def delete_all(self, auto_commit:bool= True) -> None:
        ...
        try:
            ...
            if auto_commit:
                self.db_session.commit()
            self.log.info("delete_all_products done")
        except Exception as e:
    ...
    def get_product_by_sub_category_id(self, sub_category_id:int) -> list:
        ...
        try:
            products = (
                self.db_session.query(ModelProduct)
                .filter_by(sub_category_id=sub_category_id)
                .all()
            )
            self.log.info("get_product_by_sub_category_id done")
            return products
        except Exception as e:
    ...
    def get_by_api_id(self, api_id:str) -> ModelProduct:
        """Get product by api_id."""
        try:
            product = (
                self.db_session.query(ModelProduct).filter_by(api_id=api_id).first()
            )
            self.log.info("get_product_by_api_id done")
            return product
        except Exception as e:
    ...
    def get_all(self) -> List[Any]:
        ...
        try:
            ...
            self.log.info("get_all_products done")
            return products
        except Exception as e:
    ...
    def get_by_name(self, name:str) -> ModelProduct:
        """Get product by name."""
        try:
            product = self.db_session.query(ModelProduct).filter_by(name=name).first()
            self.log.info(f"get_product_by_name done")
            return product
        except Exception as e:
            messageException = f"file: setup_db.py method:, get_product_by_name fail {e}"
            self.log.error(messageException)
            raise Exception(messageException)
    
    def get_by_upc_barcode(self, upc_barcode:str) -> ModelProduct:
        """Get product by upc_barcode."""
        try:
            product = self.db_session.query(ModelProduct).filter_by(upc_barcode=upc_barcode).first()
            self.log.info(f"get_product_by_upc_barcode done")
            return product
        except Exception as e:
            messageException = f"file: setup_db.py method:, get_product_by_upc_barcode fail {e}"
            self.log.error(messageException)
            raise Exception(messageException)
