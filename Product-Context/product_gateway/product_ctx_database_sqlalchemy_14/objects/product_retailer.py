...
class ProductRetailerDataBase(ObjectDataBase):
    ...
    def update(self, product_retailer:ModelProductRetailer, auto_commit:bool = True)->ModelProductRetailer:
        
        try:
            self.db_session.add(product_retailer)
            if auto_commit:
                self.db_session.commit()
            else:
                self.db_session.flush()
            self.db_session.refresh(product_retailer)
            return product_retailer
        
        except Exception as e:
            self.log.error(f"Error updating product retailer: {e}")
            raise Exception(f"Error updating product retailer: {e}")
    ...
    def get_by_product_id_and_retailer_id(self, product_id:int, retailer_id:int)->ModelProductRetailer:
        try:
            product_retailer = self.db_session.query(ModelProductRetailer).filter(ModelProductRetailer.product_id == product_id).filter(ModelProductRetailer.retailer_id == retailer_id).first()
        except Exception as e:
            self.log.error(f"Error getting product retailer: {e}")
            raise Exception(f"Error getting product retailer: {e}")
